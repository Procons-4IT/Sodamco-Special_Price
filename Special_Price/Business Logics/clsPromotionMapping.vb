Public Class clsPromotionMapping
    ...
    Public Sub LoadForm()
        Try
            oForm = oApplication.Utilities.LoadForm(xml_OCPR, frm_OCPR)
...
#Region "Item Event"
    Public Overrides Sub ItemEvent(ByVal FormUID As String, ByRef pVal As SAPbouiCOM.ItemEvent, ByRef BubbleEvent As Boolean)
        Try
            If pVal.FormTypeEx = frm_OCPR Then
                            ...
                            Case SAPbouiCOM.BoEventTypes.et_CHOOSE_FROM_LIST
                                ...
                                Try
                                    ...
                                    ElseIf pVal.ItemUID = "12" Then
                                        strValue = oDataTable.GetValue(CType(oForm.Items.Item(pVal.ItemUID).Specific, SAPbouiCOM.EditText).ChooseFromListAlias, 0)
                                        strName = oDataTable.GetValue("U_PrName", 0)
                                        Try
                                            oForm.Items.Item(pVal.ItemUID).Specific.value = strValue
                                            oForm.Items.Item("18").Specific.value = strName
                                            dtEffDate = oDataTable.GetValue("U_EffFrom", 0)
                                            dtEffTo = oDataTable.GetValue("U_EffTo", 0)
...
#Region "Function"
    ...
    Private Sub LoadPromotion(ByVal aform As SAPbouiCOM.Form)
        Try
            ...
            strqry = " Select T0.U_PrCode,T0.U_PrName,T0.U_EffFrom,T0.U_EffTo,T1.U_ItmCode,T1.U_ItmName,T1.U_Qty,T1.U_OffCode,T1.U_OffName,T1.U_OQty,T1.U_ODis "
            strqry = strqry & " From [@OPRM] T0 JOIN [@PRM1] T1 On T0.DocEntry = T1.DocEntry  "
            strqry = strqry & " Where T0.U_PrCode = '" & strPromotion & "'"
            ...
            oItemGrid.Columns.Item("U_PrCode").TitleObject.Caption = "Promotion Code"
            oItemGrid.Columns.Item("U_PrCode").Editable = False
            'oEditTextColumn.LinkedObjectType = 28

            oItemGrid.Columns.Item("U_PrName").TitleObject.Caption = "Promotion Name"
            oItemGrid.Columns.Item("U_PrName").Editable = False

            oItemGrid.Columns.Item("U_EffFrom").TitleObject.Caption = "Effective From"
            oItemGrid.Columns.Item("U_EffFrom").Editable = False

            oItemGrid.Columns.Item("U_EffTo").TitleObject.Caption = "Effective To"
            oItemGrid.Columns.Item("U_EffTo").Editable = False

            oItemGrid.Columns.Item("U_ItmCode").TitleObject.Caption = "Item Code"
            oItemGrid.Columns.Item("U_ItmCode").Type = SAPbouiCOM.BoGridColumnType.gct_EditText
            oEditTextColumn = oItemGrid.Columns.Item("U_ItmCode")
            oEditTextColumn.LinkedObjectType = "4"
            oItemGrid.Columns.Item("U_ItmCode").Editable = False


            oItemGrid.Columns.Item("U_ItmName").TitleObject.Caption = "Item Name"
            oItemGrid.Columns.Item("U_ItmName").Editable = False

            oItemGrid.Columns.Item("U_Qty").TitleObject.Caption = "Quantity"
            oItemGrid.Columns.Item("U_Qty").Editable = False
            oItemGrid.Columns.Item("U_Qty").RightJustified = True

            oItemGrid.Columns.Item("U_OffCode").TitleObject.Caption = "Offer Item"
            oItemGrid.Columns.Item("U_OffCode").Type = SAPbouiCOM.BoGridColumnType.gct_EditText
            oEditTextColumn = oItemGrid.Columns.Item("U_OffCode")
            oEditTextColumn.LinkedObjectType = "4"
            oItemGrid.Columns.Item("U_OffCode").Editable = False

            oItemGrid.Columns.Item("U_OffName").TitleObject.Caption = "Offer Name"
            oItemGrid.Columns.Item("U_OffName").Editable = False

            oItemGrid.Columns.Item("U_OQty").TitleObject.Caption = "Offer Qty"
            oItemGrid.Columns.Item("U_OQty").Editable = False
            oItemGrid.Columns.Item("U_OQty").RightJustified = True

            oItemGrid.Columns.Item("U_ODis").TitleObject.Caption = "Offer Discount % "
            oItemGrid.Columns.Item("U_ODis").Editable = False
            oItemGrid.Columns.Item("U_ODis").RightJustified = True
    ...
    Private Function InsertPromotionMapping(ByVal aform As SAPbouiCOM.Form) As Boolean
        ...
        Try
            ...
            oOCPR = oApplication.Company.UserTables.Item("OCPR")
            oCustomerGrid = oForm.Items.Item("11").Specific
            oItemGrid = oForm.Items.Item("14").Specific
            ...
            For index As Integer = 0 To oCustomerGrid.Rows.Count - 1
                If oCustomerGrid.DataTable.GetValue("Select", index) = "Y" Then
                    For indexItem As Integer = 0 To oItemGrid.Rows.Count - 1
                        Dim intStatus As Integer
                        Dim intCode As Integer = oApplication.Utilities.getMaxCode("@OCPR", "Code")
                        oOCPR.Code = intCode.ToString()
                        oOCPR.Name = intCode.ToString()
                        oOCPR.UserFields.Fields.Item("U_PrCode").Value = oItemGrid.DataTable.GetValue("U_PrCode", indexItem)
                        oOCPR.UserFields.Fields.Item("U_CustCode").Value = oCustomerGrid.DataTable.GetValue("CardCode", index)
                        oOCPR.UserFields.Fields.Item("U_EffFrom").Value = oItemGrid.DataTable.GetValue("U_EffFrom", indexItem)
                        oOCPR.UserFields.Fields.Item("U_EffTo").Value = oItemGrid.DataTable.GetValue("U_EffTo", indexItem)
                        oOCPR.UserFields.Fields.Item("U_ItmCode").Value = oItemGrid.DataTable.GetValue("U_ItmCode", indexItem)
                        oOCPR.UserFields.Fields.Item("U_ItmName").Value = oItemGrid.DataTable.GetValue("U_ItmName", indexItem)
                        oOCPR.UserFields.Fields.Item("U_Qty").Value = oItemGrid.DataTable.GetValue("U_Qty", indexItem)
                        oOCPR.UserFields.Fields.Item("U_OffCode").Value = oItemGrid.DataTable.GetValue("U_OffCode", indexItem)
                        oOCPR.UserFields.Fields.Item("U_OffName").Value = oItemGrid.DataTable.GetValue("U_OffName", indexItem)
                        oOCPR.UserFields.Fields.Item("U_OQty").Value = oItemGrid.DataTable.GetValue("U_OQty", indexItem)
                        oOCPR.UserFields.Fields.Item("U_ODis").Value = oItemGrid.DataTable.GetValue("U_ODis", indexItem)
                        intStatus = oOCPR.Add()
                        If intStatus <> 0 Then
                            _retVal = False
    ...
    Private Sub reDrawForm(ByVal oForm As SAPbouiCOM.Form)
        Try
            ...
            oForm.Items.Item("14").Height = (oForm.Height - 150) / 2
